...
$filter = "*Windows Compatible*"
Get-ChildItem $downloadLocation -Recurse -Filter $filter | ForEach-Object {
	$fontFiles.Add($_)
}

$fontFilesDeDup = $fontFiles | Group-Object -Property BaseName | ForEach-Object { $_.Group[0] }

New-Item -Path "$downloadLocation\all" -ItemType Directory
foreach ($fontFile in $fontFilesDeDup) {
	Copy-Item $fontFile.FullName -Destination "$downloadLocation\all"
}
