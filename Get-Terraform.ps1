<#
.SYNOPSIS
    Update the local executable terraform.exe
...
.EXAMPLE
    ...
    You already have the latest version of Terraform!

    .EXAMPLE
    ❯ Get-Terraform -LocalPath C:\HashiCorp
    Your version of Terraform is out of date!
    Downloading Terraform 0.13.5 for amd64!
#>
...
function Get-Terraform {
    ...
    switch ($env:PROCESSOR_ARCHITECTURE) {
        "AMD64" { $tf_arch = 'amd64' }
        "x86" { $tf_arch = '386' }
        default { throw "Terraform package for OS architecture '$_' is not supported." }
    }
    ...
    $tf_release_url = 'https://releases.hashicorp.com/terraform/index.json'
    ...
    $command = "$LocalPath" + "terraform.exe"
    ...
    if ($installedVersion -ne $latestVersion) {
        Write-Output "Your version of Terraform is out of date!"
        $url = "https://releases.hashicorp.com/terraform/$($latestVersion)/terraform_$($latestVersion)_windows_$tf_arch.zip"
        $temp = ([System.IO.Path]::GetTempPath()).TrimEnd("\")
        $zipfile = "$temp\terraform.zip"
        $zipfolder = "$temp\terraform\"
        ...
        Write-Output "Downloading Terraform $latestVersion for $tf_arch!"
        ...
        Copy-Item -Path "$zipfolder\terraform.exe" -Destination $LocalPath -Force
        ...
        Write-Output 'You already have the latest version of Terraform!'
    }
